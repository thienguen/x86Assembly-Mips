...
STRLENGTH	equ	32			; length of string
...
; CHECK ARGV[4] != open
	...
	mov rsi, O_RDWR					; rsi = permission read / write
	syscall							; system services verify the permisson
	cmp rax, 0						; fail mean rax would be < 0
	jl _errOpenOut
...
divideLoop:
	...
	cmp rcx, STRLENGTH				; control flow
	jne divideLoop
...
; --- IF -----------------------------------------------------------------
getNextChar:
	cmp r14, r15					;* if (currIdx >= buffMax)
	je	readFile					
...
; -----
	...
	mov r12, rdi 					; r12 = file descriptor

	mov	rdx, 0						; Count characters to write
w_strCountLoop:
...
;  Call OS to output string.
	...
	mov	rsi, rbx					; address of char to write into
	mov	rdi, r12 					; destiation that we writeing to
	syscall							; system call
