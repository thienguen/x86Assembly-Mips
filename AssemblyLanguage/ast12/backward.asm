...
loopTrue:								; loop to sum divisors
	...
	cmp rbx, 1							; if N == 1, exlcude it out!
	je loopTrue							; 1 is not any number, its fake

	xor r13, r13						; r13 = sum of divisors
	mov ecx, ebx						; ecx = N (current number)
										; Loop until we reach 0

forLoop:

	cmp ecx, 1							; already included 1 in the sum
	je forLoopEnd						;* If ( i == 1 )

	dec ecx

	mov eax, ebx						; rax = N (current number)
	mov edx, 0
	div ecx								; rax = N / counter

	cmp edx, 0							;* if (n % i == 0)
	jne forLoop							; if so, add counter to sum

	add r13d, ecx						;* sum = sum + i;
	jmp forLoop

forLoopEnd:

	cmp r13d, ebx						; check if sum of divisors == N
